const generatorScriptPrefix = String.raw`
...
const generatorScriptSuffix = String.raw`                }
...
const searchBootstrapUrl = 'https://api.scryfall.com/cards/search?order=released&q=type%3Dplaneswalker';

const generatorScriptVariants = {
  download: {
    collectRows: String.raw`                    allAbilities.forEach(function(rowArray) {
                        csvContent += rowArray.join(";") + "$$$";
                    })
`,
    exportRows: String.raw`                    var encodedUri = encodeURI(csvContent)
                    ...
                    download.click()
`,
    bootstrap: `    importPlaneswalkers("${searchBootstrapUrl}")`,
  },
  'window-open': {
    collectRows: String.raw`                    allAbilities.forEach(function(rowArray) {
                                         csvContent += rowArray.join(";") + "$$$";
...
export function askUrzaAbilityGeneratorScript(variant = 'download') {
  const scriptVariant = generatorScriptVariants[variant];

  if (!scriptVariant) {
    throw new Error(`Unknown Ask Urza ability generator variant: ${variant}`);
  }

  return [
    generatorScriptPrefix,
    scriptVariant.collectRows,
    scriptVariant.exportRows,
    generatorScriptSuffix,
    scriptVariant.bootstrap,
  ].join('');
}
